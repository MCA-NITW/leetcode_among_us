...
export const fetchBatchUserData = async (usernames) => {
  try {
    const response = await fetch(`${getApiBaseUrl()}/leetcode/batch-user-data`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
export const processUserDataResponse = (leetcoder, backendData) => {
  ...
  try {
    ...
    const contestHistory = userContestRankingInfo.data.userContestRankingHistory || []
    ...
    const getAverageContestRanking = (contestHistory) => {
      let totalRanking = 0, totalContest = 0
      ...
      return totalContest === 0 ? Infinity : Math.round(totalRanking / totalContest)
    }

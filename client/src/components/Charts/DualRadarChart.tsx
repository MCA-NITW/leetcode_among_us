...
import './Charts.css'
import type { TagProblemCounts } from '../../types'
...
interface DualRadarChartProps {
  user1Tags?: TagProblemCounts
  user2Tags?: TagProblemCounts
  user1Name?: string
  user2Name?: string
}
...
function DualRadarChart({
  ...
  const { labels, user1Values, user2Values, hasData } = useMemo(() => {
    ...
    const flatten = (tags?: TagProblemCounts) => {
      if (!tags) return []
      const all = [
        ...(tags.advanced || []),
        ...(tags.intermediate || []),
    ...
    if (flat1.length === 0 && flat2.length === 0) {
      return {
        labels: [] as string[],
        user1Values: [] as number[],
        user2Values: [] as number[],
        hasData: false
      }
    }
    ...
    commonTags.sort(
      (a, b) => map1.get(b)! + map2.get(b)! - (map1.get(a)! + map2.get(a)!)
    )

...
interface DualRatingChartProps {
  user1History?: ContestEntry[]
  user2History?: ContestEntry[]
  user1Name?: string
  user2Name?: string
}
 ...
 */
function DualRatingChart({
  user1History,
  user2History,
  user1Name,
  user2Name
}: DualRatingChartProps) {
  ...
  const { labels, user1Ratings, user2Ratings, hasData } = useMemo(() => {
    ...
    const filterAndSort = (history?: ContestEntry[]) =>
      (history || [])
    ...
    if (sorted1.length === 0 && sorted2.length === 0) {
      return {
        labels: [] as string[],
        user1Ratings: [] as (number | null)[],
        user2Ratings: [] as (number | null)[],
        hasData: false
      }
    }

...
interface ContestScatterChartProps {
 ...
 */
function ContestScatterChart({
  contestHistory = []
}: ContestScatterChartProps) {
  ...
  const pointColor = (solved: number): string => {
        ...
        tooltip: {
          ...
          callbacks: {
            label: (ctx: {
              ...
              return [
                `Rating: ${ctx.parsed.x ?? 0}`,
                `Solved: ${ctx.parsed.y ?? 0}`,
                title
              ]
            }
          }
